...
namespace
{
  ...
  FileHandle openFile(int fd, bool readOnly)
  {
    return fdopen(fd, readOnly ? "rb" : "rb+");
  }
...
{
  if (!openReadOnly)
    d->file = openFile(fd, false);

  if (d->file != InvalidFileHandle)
    d->readOnly = false;
  else
    d->file = openFile(fd, true);

  if (d->file == InvalidFileHandle)
  {
    debug("Could not open file with file descriptor");
  }
}
